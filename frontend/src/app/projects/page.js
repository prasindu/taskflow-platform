  ...
  return (
    <AppShell>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="font-display text-3xl font-bold flex items-center gap-3">
            <LayoutTemplate className="text-[var(--teal)]" size={32} /> Projects
          </h1>
          <p className="text-[var(--text-dim)] mt-1">Manage and track all your team's projects.</p>
        </div>
        {canCreate && (
      ...
      {loading ? (
        ...
        <div className="panel p-16 text-center flex flex-col items-center justify-center border-dashed border-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((p) => (
            <Link key={p.id} href={`/projects/${p.id}`} className="panel p-6 block hover:border-[var(--teal)] hover:shadow-lg hover:-translate-y-1 transition-all group relative">
              <div className="flex items-start justify-between mb-3">
                <h3 className="font-display font-semibold text-lg group-hover:text-[var(--teal)] transition-colors pr-4">{p.name}</h3>
                <span className={`px-2 py-1 rounded text-[10px] font-bold tracking-wider border ${getStatusStyles(p.status)}`}>
                  {p.status}
                </span>
              </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="panel w-full max-w-lg p-0 overflow-hidden shadow-2xl border-[var(--border)] animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={createProject} className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
              <div>
                ...
                <div className="mt-1 max-h-48 overflow-y-auto border border-[var(--border)] rounded-lg bg-[var(--bg)] divide-y divide-[var(--border)]">
                  {members.map((m) => (
                    <label key={m.id} className="flex items-center gap-3 p-3 hover:bg-[var(--bg-panel-hover)] cursor-pointer transition-colors">
                      ...
                      <div className="w-8 h-8 rounded-full bg-[var(--bg-panel)] border border-[var(--border)] flex items-center justify-center text-xs font-bold">{getInitials(m.name)}</div>
            ...
            <div className="p-5 border-t border-[var(--border)] bg-[var(--bg)] flex gap-3 justify-end">
              <button type="button" onClick={() => setShowModal(false)} className="btn-secondary px-6">Cancel</button>
              <button type="submit" disabled={saving} onClick={createProject} className="btn-primary px-8">{saving ? "Creating..." : "Create Project"}</button>
            </div>
          </div>
        </div>
      )}

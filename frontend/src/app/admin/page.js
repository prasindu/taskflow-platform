      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl font-bold mb-2 flex items-center gap-3">
            <Shield className="text-[var(--gold)]" size={32} /> User Management
          </h1>
          <p className="text-[var(--text-dim)]">Approve new accounts and manage system roles securely.</p>
        </div>
      </div>

      
      <section className="mb-10">
        <h2 className="font-display text-lg font-semibold mb-4 flex items-center gap-2 text-orange-500">
        ...
        <div className={`panel p-0 overflow-hidden ${pending.length > 0 ? 'border-orange-500/30 shadow-[0_0_15px_rgba(249,115,22,0.05)]' : ''}`}>
          {pending.length === 0 ? (
            <div className="p-10 text-center text-[var(--text-dim)] flex flex-col items-center">
            ...
            <div className="divide-y divide-[var(--border)]">
              {pending.map((u) => (
                <div key={u.id} className="p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-[var(--bg)] hover:bg-[var(--bg-panel-hover)] transition-colors">
        ...
        <div className="panel p-0 overflow-hidden border-[var(--border)]">
          <div className="divide-y divide-[var(--border)] max-h-[600px] overflow-y-auto">
            {filteredUsers.length === 0 ? (
              ...
              filteredUsers.map((u) => (
                <div key={u.id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-[var(--bg-panel-hover)] transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[var(--bg)] text-[var(--text-dim)] flex items-center justify-center font-bold text-sm border border-[var(--border)]">
                      {getInitials(u.name)}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                    {/* Status Badge */}
                    {!u.isApproved ? (
                      <span className="px-2 py-1 rounded-md text-[10px] font-bold tracking-wider bg-orange-500/10 text-orange-500 border border-orange-500/20">
                        PENDING
                      </span>
                    ) : (
                    ...
                    <select
                      className="input-field text-sm w-36 bg-[var(--bg)] py-2"
                      value={u.role || ""}
